...
const generateDefaultProjectSchema = () => {
  return generateProjectSchema(DefaultPageSchema, DefaultI18nSchema);
};

export const generateNewDefaultConfig = async () => {
  project.importSchema(generateDefaultProjectSchema() as any);
  return {
...
export const resetSchema = async (scenarioName: string = 'unknown') => {
  ...
  const defaultSchema = generateDefaultProjectSchema();
