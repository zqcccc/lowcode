...
import appHelper, { request } from './appHelper';
import {
...
const getId = function () {
  return new URLSearchParams(location.search.slice(1)).get('id');
};
...
const SamplePreview = () => {
  ...
  async function init() {
    ...
    if (id) {
      ...
      const { config, name } = await res.data;
      const { packages: _packages, projectSchema: _projectSchema } = config;
      packages = _packages;
